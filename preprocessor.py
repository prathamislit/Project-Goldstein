    ...
    if os.path.exists(region_master):
        try:
            ...
            if len(existing_prior) > 0:
                ...
                # ── Schema Lock Assertion (Configuration Mutation Protection)
                expected_ret_col = f"{sector_etf}_log_return"
                if expected_ret_col not in existing_prior.columns:
                    msg = (
                        f"Config Mutation Detected: You mapped {config.ACTIVE_REGION} to ETF '{sector_etf}', "
                        f"but the historical cache lacks '{expected_ret_col}'. "
                        "Incremental runs are mathematically invalid across config changes! "
                        "Please delete the cache and run a FULL backfill."
                    )
